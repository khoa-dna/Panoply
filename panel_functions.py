...
def create_marker_strand(marker_name_list, abundance_list) -> Marker_strand:
    ...
    for i, marker_name in enumerate(marker_name_list):
        marker_strand.add_marker(Marker(name= marker_name, abundance= abundance_list[i]))
...
def create_fluor_strand(fluor_name_list, brightness_list, channel_values) -> Fluor_strand:
    ...
    for i, fluor_name in enumerate(fluor_name_list):
        fluor_strand.add_fluor(Fluor(name= fluor_name, brightness= brightness_list[i], channel_values = channel_values[i]))
...
def create_dna(fluor_strand, marker_strand, bond_fluor_name_list,\
    ...
    for i, val in enumerate(bond_fluor_name_list):
        if bond_fluor_name_list[i] in fluor_keys and bond_marker_name_list[i] in marker_keys:
            ...
            bond = Bond(fluor_end= fluor_end, marker_end=marker_end, state = "available")
    ...
    std_strength = (np.array(strength_list) - np.mean(strength_list)) / np.std(strength_list)
    count = 0
    for bonds in dna.all_bonds:
        for b in bonds:
...
def calculate_composite_score(markers, fluors, new_bond, auto_fluor, **kwargs):
    ...
    return ci + kwargs["alpha"]*abs(strength) + kwargs["beta"]*corr - 0.2*np.log(new_bond.fluor_end.brightness)
...
def choose_next_panel(panel, min_marker, branch_num, random_prob, auto_fluor, **kwargs):
    ...
    for i in range(min(branch_num,len(sorted_table))):
        if random.random() > random_prob:
            ...
            best_list.append(sorted_table.pop(random.randrange(len(sorted_table))))
...
def grow_panel(panel, branch_num, random_prob, auto_fluor, **kwargs):
    ...
    if len(min_marker.get_available_bonds()) == 0:
        ...
        panels = choose_next_panel(panel, min_marker, branch_num, random_prob, auto_fluor,  alpha = kwargs["alpha"], beta = kwargs["beta"])
    return panels

def grow_frontier(frontier, frontier_size, branch_num, random_prob, risk_prob, auto_fluor, **kwargs):
    for panel in frontier.get_panel_list():
        new_panel_list = []
        new_panels = grow_panel(panel, branch_num, random_prob, auto_fluor, alpha = kwargs["alpha"], beta = kwargs["beta"])
        if new_panels != None:
            for p in new_panels:
                if p != None:
        ...
        while (len(new_panel_list) < frontier_size) and len(frontier.reserve) > 0:
            ...
            new_ps = grow_panel(p, branch_num, random_prob, auto_fluor,  alpha = kwargs["alpha"], beta = kwargs["beta"])
            for p in new_ps:
                new_panel_list.append(new_p)
                
    selected_panels, unselected_panels = choose_best(frontier_size, risk_prob, new_panel_list)
...
def panel_data_preprocessing(abundance_df, brightness_df, target_df, spectra_df, database_df):
    ...
    deepcopy(abundance_df), deepcopy(brightness_df), deepcopy(target_df), deepcopy(spectra_df), deepcopy(database_df)
    database_df_copy = filter_database(database_df_copy, spectra_df_copy, target_df_copy)
    marker_name_list, marker_abundance_list = generate_marker_data(abundance_df_copy, target_df_copy)
    fluor_name_list, fluor_brightness_list, fluor_channel_values_list = generate_fluor_data(brightness_df_copy, database_df_copy, spectra_df_copy)
...
def filter_database(database_df, spectra, target_markers_df):
    ...
    for i,val in enumerate(database_df["Fluor"].values):
        if val in spectra.columns: #and val in fluor_brightness_df["Fluor"].values
    ...
    intersection_list = list(set(fluor_index_list).intersection(marker_index_list))
...
def generate_marker_data(marker_abundance_df, target_markers_df):
    ...
    for i in marker_name_list:
        ...
        if i not in marker_abundance_df["Marker"].values:
            ...
            abundance = marker_abundance_df[marker_abundance_df["Marker"] == i]["Abundance"].values[0]
...
def generate_fluor_data(fluor_brightness_df, database_df, spectra):
    ...
    for i in fluor_name_list:
        ...
        if i not in fluor_brightness_df["Fluor"].values:
            ...
            brightness = fluor_brightness_df[fluor_brightness_df["Fluor"] == i]["Brightness"].values[0]
...
def calculate_panel_approx_score(markers, fluors, auto_fluor, \
    ...
    for i in fluors:
        fluor_brightness.append(i.brightness)
    min_strength = min(np.log(marker_abundances)) * min(np.log(fluor_brightness))
    corr = pearson_corr(marker_abundances, fluor_brightness)
