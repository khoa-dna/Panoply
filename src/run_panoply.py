...
class Input_Frame(ttk.Frame):
    def __init__(self, parent):
        ...
        self.abundance_button = ttk.Button(self,
                                           ...
                                           command=lambda: self.button_load_file(
                                               "Abundance"),
                                           width=CONFIG_PARAM["load_button_width"])
...
class Options_Frame(ttk.Frame):
    def __init__(self, parent):
        ...
        plot_approx_button = ttk.Button(self, text="Plot Approx Score", command=lambda: self.plot_approx(
            float(intensity_eval_weight.get()), float(corr_eval_weight.get())))
        plot_ci_button = ttk.Button(
            self, text="Plot Complexity Index", command=self.plot_ci)
        plot_ci_button.grid(row=0, column=0)
    ...
    def plot_approx(self, intensity_eval_weight, corr_eval_weight):
        if self.controller:
            self.controller.plot_approx(
                intensity_eval_weight, corr_eval_weight)

    def plot_ci(self):
        if self.controller:
            self.controller.plot_ci()
...
class Download_Frame(ttk.Frame):
    def __init__(self, parent):
        ...
        self.controller = None
        self.download_panel_button = ttk.Button(
            self, text="Download Panels", command=self.download_panel)
    ...
    def download_panel(self):
        ...
        if not output_name:
            output_name = f"panel_{str(datetime.datetime.now())}".replace(
                " ", "-").replace(".", "-").replace(":", "-")
    ...
    def plot(self, plot_type, data, **kwargs):
        ...
        if plot_type == "Line":
            ...
            if "sort" in kwargs.keys():
                ...
                df = df.sort_values(
                    "Value", ascending=True).reset_index(drop=True)
                for i in range(len(df)):
                    axes.text(y=df.Value[i], x=i,
                              s="#"+str(df["index"][i]), size=10)
                axes.plot(df.Value)
...
class Controller:
    ...
    def load_config_file(self):
        try:
            ...
            with open(config_path, 'r') as fp:
                config = json.load(fp)
            assert "Panel_Config" == list(config.keys())[
                0], "Not a Panel Config File"
            ...
            self.view.param_frame.intensity_weight.set(
            ...
            self.view.param_frame.intensity_factor.set(
            ...
            self.view.param_frame.positive_fraction.set(
    ...
    def load_input(self, file_type, path):
        ...
        if extension == "csv":
            ...
            showerror(title='Error',
                      message='FIle extensions must be .csv or .xlsx')
            return

        if file_type == "Abundance":
            if self.check_abundance_format(df):
                self.model.file_inputs[file_type] = df
                self.model.file_input_paths[file_type] = path
                self.view.input_frame.abundance_file_label.config(
                    text="Abundance file: " + base_file_name)
        elif file_type == "Brightness":
            if self.check_brightness_format(df):
                self.model.file_inputs[file_type] = df
                self.model.file_input_paths[file_type] = path
                self.view.input_frame.brightness_file_label.config(
                    text="Brightness file: " + base_file_name)
        elif file_type == "Target":
            if self.check_target_format(df):
                self.model.file_inputs[file_type] = df
                self.model.file_input_paths[file_type] = path
                self.view.input_frame.target_file_label.config(
                    text="Target file: " + base_file_name)
        elif file_type == "Spectra":
            if self.check_spectra_format(df):
                self.model.file_inputs[file_type] = df
                self.model.file_input_paths[file_type] = path
                self.view.input_frame.spectra_file_label.config(
                    text="Spectra file: " + base_file_name)
        elif file_type == "Database":
            if self.check_database_format(df):
                self.model.file_inputs[file_type] = df
                self.model.file_input_paths[file_type] = path
                self.view.input_frame.database_file_label.config(
                    text="Database file: " + base_file_name)

    def check_abundance_format(self, df):
        try:
            assert len(
                df.columns) == 2, "Abundance file must contains 2 columns"
    ...
    def check_brightness_format(self, df):
        try:
            assert len(
                df.columns) == 2, "Brigtness file must contains 2 columns"
    ...
    def search(self, panel_num):
        ...
        try:
            assert len(self.model.file_inputs.values()
                       ) == 5, "Missing input files"
            assert len(self.model.params.keys()) >= 6, "Missing parameters"

            params = self.model.params
            abundance_df, brightness_df, target_df, spectra_df, database_df = self.model.file_inputs.values()

            database_df, marker_name_list, marker_abundance_list,             fluor_name_list, fluor_brightness_list, fluor_channel_values_list = panel_data_preprocessing(
                abundance_df, brightness_df, target_df, spectra_df, database_df)

            start = time.time()
            self.afhulym = Fluor(name=AFHULYM, brightness=1,
                                 channel_values=spectra_df[AFHULYM].values)
            real_marker_strand = create_marker_strand(
                marker_name_list, marker_abundance_list)
            real_fluor_strand = create_fluor_strand(fluor_name_list,
                                                    fluor_brightness_list, fluor_channel_values_list)
            real_dna = create_dna(real_fluor_strand, real_marker_strand,
                                  database_df["Fluor"].values, database_df["Marker"].values,                                 marker_abundance_list, fluor_brightness_list)
            results = constrained_beam_search(init_panel=real_dna, result_num=panel_num,                                               frontier_size=params["frontier_size"], branch_num=params["branch_num"],                                              random_prob=params[
                                              "random_prob"], risk_prob=params["risk_prob"],                                               auto_fluor=self.afhulym, alpha=params["intensity_weight"], beta=params["corr_weight"])

            end = time.time()
            print('{:.4f} s'.format(end-start))
            panels_flattened = [i for b in map(lambda x:[x] if not isinstance(
                x, list) else x,                                               results) for i in b]
            self.model.panels = panels_flattened

            self.model.panel_scores = self.calculate_approx_scores(
                self.model.panels, self.model.params["intensity_eval_weight"],                                                              model.params["corr_eval_weight"])

            self.model.panel_file = self.generate_panel_file()
    ...
    def calculate_approx_scores(self, panels, intensity_eval_weight, corr_eval_weight):
        panel_scores = []
        for panel in panels:
            score = calculate_panel_approx_score(markers=panel.marker_history, fluors=panel.fluor_history,
                                                 auto_fluor=self.afhulym, intensity_eval_weight=intensity_eval_weight, corr_eval_weight=corr_eval_weight)
    ...
    def plot_approx(self, intensity_eval_weight, corr_eval_weight):
        ...
        self.model.panel_scores = self.calculate_approx_scores(
            self.model.panels, intensity_eval_weight, corr_eval_weight)
    ...
    def plot_ci(self):
        try:
            ...
            for panel in self.model.panels:
                full_fluor_list = panel.fluor_history + [self.afhulym]
                cis.append(calculate_ci(full_fluor_list))
                print(full_fluor_list)
            self.model.ci_scores = cis
            self.view.canvas_frame.plot(plot_type="Line", data=self.model.ci_scores,
                                        sort=True,
                                        index=np.arange(len(self.model.ci_scores)))
        except Exception as error:
            showerror(title='Error', message=error)
            return
    ...
    def unmix(self, unmix_num):
        ...
        method = "Ridge"
        params = self.model.params
        self.model.unmixed_samples = []
        self.model.panel_recalls = []
        abundance_df, brightness_df, target_df, spectra_df, database_df = self.model.file_inputs.values()
        panel_and_score = tuple(
            zip(self.model.panels, self.model.panel_scores))
        ...
        for i, panel in enumerate(panel_sorted[:unmix_num]):
            sample = Sample(ID=i, cell_num=params["sample_size"], dna=panel, channel_num=len(
                self.model.file_inputs["Spectra"]))
            # Add each flour to 20% cells in sample:
            for fluor in panel.fluor_history:
                # Add 20%
                chosen_cells = np.random.choice(sample.cells,                                                 int(
                    params["positive_fraction"]*params["sample_size"]), replace=False)
                for cell in chosen_cells:
                    intensity_factor = np.random.normal(0, 1)
                    intensity_factor = np.exp(
                        intensity_factor/2) * params["intensity_factor"]
                    cell.add_fluor(
                        fluor=fluor, intensity_factor=intensity_factor)
            ...
            for cell in sample.cells:
                ...
                cell.add_fluor(fluor=self.afhulym,
                               intensity_factor=params["auto_intensity"])

            spectra_panel = spectra_df[[
                fluor.name for fluor in panel.fluor_history]].copy()
            df_result = pd.DataFrame(
                columns=spectra_panel.columns.to_list() + ["id"])
            for i in tqdm(range(len(sample.cells))):
                measurement = sample.cells[i].emit_light(noise=params["noise"])
                unmixed_read = unmix(
                    measurement, spectra_panel, method, alpha=.1, max_iter=5000)
            ...
            for fluor in df_result.columns[:-1]:
                positive_population = set(df_result.sort_values(fluor, ascending=False)[
                                          "id"][:int(params["sample_size"]*params["positive_fraction"])].index.to_list())
                match_size = len(set(sample.components[fluor]).intersection(
                    set(positive_population)))
                recall = match_size / \
                    (int(params["sample_size"]*params["positive_fraction"]))
                recall_dict[fluor] = recall
            recall_results.append(recall_dict)

            print(recall_results, len(recall_results))
            self.model.panel_recalls = recall_results
            self.model.unmixed_samples = df_all_results
            self.model.samples = all_samples
    ...
    def generate_panel_file(self):
        ...
        for panel in panels:
            ...
            df.insert(loc=len(df.columns), column="Fluor" +
                      str(count), value=fluors)
